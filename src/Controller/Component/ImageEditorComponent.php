...
class ImageEditorComponent extends Component {
     ...
     *
     * @param int $dst_w final width
     */
    public function resizeByWidth( int $dst_w ) {
        /*
         * To be more efficient, resample large images to 5x the destination size before resizing
         * whenever the output size is less that 1/3 of the original image size.
         */
        $src_w = $this->im->getImageWidth();
        $src_h = $this->im->getImageHeight();
        ...
        if ( $src_w > $dst_w * 3 ) {
            $this->im->sampleImage( $dst_w * $sample_factor, $dst_h * $sample_factor );
        }

        // resize image
        // triangle filter seems to be best compromise between speed and quality
        // https://urmaul.com/blog/imagick-filters-comparison/
        $this->im->resizeImage( $dst_w, $dst_h, \Imagick::FILTER_TRIANGLE, 1 );
    }
